...
function validateCode(code) {
  ...
  if (openCurly !== closeCurly) issues.push(`Bracket {} tidak seimbang`);
  const openParen = (code.match(/\(/g) || []).length;
  const closeParen = (code.match(/\)/g) || []).length;
  if (openParen !== closeParen) issues.push(`Kurung () tidak seimbang`);
  const openBracket = (code.match(/\[/g) || []).length;
  const closeBracket = (code.match(/\]/g) || []).length;
  if (openBracket !== closeBracket) issues.push(`Bracket [] tidak seimbang`);
...
function detectIntent(message) {
  ...
  if (lowerMessage.includes('mode serius') || lowerMessage.includes('```')) return 'serius';
  ...
  try {
    const completion = await groq.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'Kamu adalah RHF ZERO, asisten yang ramah. Jawab SINGKAT, natural, 1-3 kalimat max. Jangan sebutkan kamu AI.',
        },
    ...
    return {
      mode: 'santai',
      response: completion.choices[0]?.message?.content || 'Halo!',
      provider: 'Groq',
    };
  } catch (error) {
...
async function handleSerius(message, startTime) {
  ...
  generateResults.forEach((result, i) => {
    ...
    if (result.status === 'fulfilled') {
      ...
      results.errors.push(name + ': gagal');
  ...
  if (successfulCodes.length > 0) {
    ...
    try {
      const reviewResult = await geminiModel.generateContent(
        `REVIEW kode berikut untuk typo, logic error, format, dan kelengkapan. PERBAIKI jika ada masalah. Output KODE FINAL dalam markdown code block.\n\nKODE:\n${bestCode}`
      );
  ...
  // ---------- FASE 3: RETE-RETE LAPIS 2 (2 AI via OpenRouter) ----------
  if (bestCode && successfulCodes.length > 0) {
    try {
      const l2Tasks = [
        openrouter.chat.completions.create({
          model: 'deepseek/deepseek-chat',
          messages: [
            { role: 'system', content: 'Verifikasi kode. Cek konsistensi, typo, dan false alarm. Output JSON: {"verified":true,"issues":[]}' },
        ...
        openrouter.chat.completions.create({
          ...
          messages: [
            { role: 'system', content: 'Verifikasi kode. Cek keamanan dan error handling. Output JSON: {"verified":true,"issues":[]}' },
            { role: 'user', content: 'Kode:\n' + bestCode.substring(0, 5000) },
          ],
          max_tokens: 1024, temperature: 0.1,
        }),
      ...
      const l2Results = await Promise.allSettled(l2Tasks);

      l2Results.forEach((r, i) => {
        const name = i === 0 ? 'DeepSeek L2' : 'Mistral L2';
        if (r.status === 'fulfilled') {
          try {
            ...
            results.l2review.push({ provider: name, verified: true });
          }
        } else {
          results.errors.push(name + ': gagal');
        }
      ...
      results.errors.push('Rete-Rete L2: ' + e.message);
    }
  }
  ...
  const l2Count = results.l2review.filter(r => r.verified !== false).length;
